...
export default function AlertDialog(props) {
       ...
       if(Number.isInteger(open.index)){ 
           let clone = JSON.parse(JSON.stringify(props.rowData))
            ...
            if(allDocuments){
                ...
                console.log('index, allDocuments, open.documentId', index, allDocuments, open.documentId)
                if(index!==-1){
  ...
  return (
    <div>
      ...
      <Dialog
        open={open.bool}
        onClose={()=>{setOpen({bool:false,index:null})}}
      ...
      >
        <DialogTitle id="alert-dialog-title"> {open.index === "document" ? "Ջնջել Փաստաթուղթը" : "Ջնջել ապրանքը"}</DialogTitle>
        <DialogContent>
            {open.index === "document" ? "Դուք համոզված ե՞ք ջնջել Փաստաթուղթը" : "Դուք համոզված ե՞ք ջնջել"}
        </DialogContent>
        <DialogActions>
        <Button
                color="secondary"
