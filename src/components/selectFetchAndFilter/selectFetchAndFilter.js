  ...
  const {inputValue, setInputValue} = props
    ...
    useEffect(()=>{
      if(inputValue && inputValue.length>2){
        setLoading(true)
        Axios.get(`${process.env.REACT_APP_API_URL}?${props.path}=${props.pathValue}/${props.param}={"${props.paramValue}":"${inputValue}"}`, {
            headers: {
    ...
    },[inputValue, props.path, props.param, props.pathValue, props.paramValue])
