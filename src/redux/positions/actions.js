...
export const getPosition = ( id ) => {
    return {
        ...
        promise: (apiClient) => apiClient.get(`User/Position`, JSON.stringify({ param:{id} }))
    }
};
...
export const addPosition = (data) => {
    ...
    let promise = (apiClient) => apiClient.post(`User/Position`,JSON.stringify({data,cols}))
    if(data.id){
        types = [EDIT_POSITION_REQUEST,EDIT_POSITION_FAIL,EDIT_POSITION_SUCCESS];
        promise = (apiClient) => apiClient.put(`User/Position`,JSON.stringify({data,cols,id:data.id}));
    }
    return {
        types: types,
        promise: promise
...
export const deletePosition = ( id ) => {
    return {
        ...
        promise: (apiClient) => apiClient.delete(`User/Position`, JSON.stringify({ id,cols }))
    }
};
